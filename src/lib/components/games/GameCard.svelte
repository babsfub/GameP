...
<script lang="ts">
  ...
  import type { GameConfig } from '$lib/types/games.js'
  ...
  async function handlePlay() {
    ...
    try {
      ...
      // Submit stake to play
      const tx = await contractActions.write.submitScore({
        account: $wallet.address,
        game: game.id,
        stake: game.minStake,
        score: BigInt(0), // Assuming initial score is 0
        scoreHash: '0x0' // Replace with actual score hash if available
      })

      // Redirect to game page after stake is submitted
      goto(game.path)
    } catch (err) {
